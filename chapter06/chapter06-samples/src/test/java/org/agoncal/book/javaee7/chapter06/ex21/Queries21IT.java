...
public class Queries21IT extends AbstractPersistentTest {
    ...
    public void shouldFindAllWithADynamicQuery() throws Exception {
        ...
        query = em.createQuery("select c from Customer21 c  where c.firstName = :fname").setParameter(
                "fname", "Vincent").setMaxResults(1);
        assertEquals(1, query.getResultList().size());
        ...
        typedQuery = em.createQuery("select c from Customer21 c  where c.firstName = :fname",
                Customer21.class).setParameter("fname", "Vincent").setMaxResults(1);
        assertEquals(1, typedQuery.getResultList().size());
...
//        assertEquals(ALL, customersNames.size());
    ...
    public void shouldExecuteSeveralDynamicQueries() throws Exception {
        ...
        query = em.createQuery("select new org.agoncal.book.javaee7.chapter06.ex21.CustomerDTO21(" +
                "c.firstName, c.lastName, c.address.country) from Customer21 c where c.firstName = 'Vincent'");
        assertEquals(2, query.getResultList().size());
    ...
    public void shouldExecuteCaseQueries() throws Exception {
        ...
        Book21 book01 = new Book21("The Hitchhiker's Guide to the Galaxy", 12F,
                "The Hitchhiker's Guide to the Galaxy is a science fiction comedy series created by Douglas Adams.",
                "1-84023-742-2", 354, false, "Apress");
        Book21 book02 = new Book21("Java EE 6", 50F, "Learn about EE 6", "2-84023-742-2",
                450, true, "Apress");
        Book21 book03 = new Book21("Narcisse and Golmund", 10F,
                "One of the best Herman Hesse book", "3-84023-742-2", 153,
                false, "Pinguin");
